...
formula1 <- "adi_tertile ~ age_grp + s(culture_year) + s(distance_km) + gender + racef + ethnicityf + isolate_sourcef + location_type"
formula2 <- "adi_tertile ~ age_grp + s(culture_year) + s(distance_km) + gender + racef + ethnicityf + isolate_sourcef"
...
generate_survival_data <- function(data, time_var, status_var, adi_var, group_var, weights_var, cluster_var, interaction_var) {
  ...
  newdata_adi <- expand.grid(
    adi_tertile = factor(c("Low", "High"), levels = levels(data[[adi_var]])),
  ...
  df_susceptible_surv <- data.frame(
    ...
    strata = factor(rep(newdata_adi$adi_tertile, each = length(surv_fit_susceptible$time)), 
  ...
  df_nonsusceptible_surv <- data.frame(
    ...
    strata = factor(rep(newdata_adi$adi_tertile, each = length(surv_fit_nonsusceptible$time)), 
                    levels = levels(data[[adi_var]])),
...
ggplot(df_mdr_all, aes(x = ps, fill = adi_tertile)) +
  geom_density(alpha = 0.5) +
  theme_minimal() +
  scale_fill_brewer(palette = "Set1")

balance <- bal.tab(
  adi_tertile ~ age_grp + rcs(culture_year, 5) + rcs(distance_km, 5) + gender + racef + ethnicityf + isolate_sourcef + location_type,
  data = df_mdr_all,
...
df_mdr_prop <- df_mdr_all |>
  select(isolate_id, zip_code, mdr, adi_tertile) |>
...
```{r}
chart_prev_mdr <- ggplot(df_mdr_prop, aes(x = adi_tertile, y = prop)) +
  geom_boxplot() +
...
ggplot(df_amino, aes(x = ps, fill = adi_tertile)) +
  geom_density(alpha = 0.5) +
  theme_minimal() +
  scale_fill_brewer(palette = "Set1")
...
levels(df_surv$group) <- c("Low ADI, non-MDR", "High ADI, non-MDR", 
                   "Low ADI, MDR", "High ADI, MDR")
```
...
df_amino_prop <- df_amino |>
  select(isolate_id, zip_code, resistant_to_class, adi_tertile) |>
...
```{r}
chart_prev_amino <- ggplot(df_amino_prop, aes(x = adi_tertile, y = prop)) +
  geom_boxplot() +
  geom_jitter(width = 0.2, alpha = 0.6) +
  scale_y_continuous(limits = c(0, .6, .2)) +
  labs(
...
df_carba |>
  tabyl(adi_tertile, day1, resistant_to_class) |>
  adorn_totals(c("row", "col")) |>
  adorn_percentages() |>
  adorn_pct_formatting() |>
  adorn_ns() |>
...
ggplot(df_carba, aes(x = ps, fill = adi_tertile)) +
  geom_density(alpha = 0.5) +
  theme_minimal() +
  scale_fill_brewer(palette = "Set1")
...
cox_carb_susc <- coxph(Surv(ftime, status) ~ adi_tertile * day1,
                 weights = iptw,
                 data = df_carba |> filter(!resistant_to_class))
...
```{r}
ggplot(df_carba_prop, aes(x = adi_tertile, y = prop)) +
  geom_boxplot() +
  geom_jitter(width = 0.2, alpha = 0.6) +
  labs(
    x = "ADI tertile",
...
ggplot(df_ceph, aes(x = ps, fill = adi_tertile)) +
  geom_density(alpha = 0.5) +
  theme_minimal() +
  scale_fill_brewer(palette = "Set1")
...
```{r}
chart_prev_ceph <- ggplot(df_ceph_prop, aes(x = adi_tertile, y = prop)) +
  geom_boxplot() +
  geom_jitter(width = 0.2, alpha = 0.6) +
  scale_y_continuous(limits = c(0, .6, .2)) +
  labs(
...
ggplot(df_penbli, aes(x = ps, fill = adi_tertile)) +
  geom_density(alpha = 0.5) +
  theme_minimal() +
  scale_fill_brewer(palette = "Set1")
...
```{r}
chart_prev_penbli <- ggplot(df_penbli_prop, aes(x = adi_tertile, y = prop)) +
  geom_boxplot() +
  geom_jitter(width = 0.2, alpha = 0.6) +
  scale_y_continuous(limits = c(0, .6, .2)) +
  labs(
...
ggplot(df_quin, aes(x = ps, fill = adi_tertile)) +
  geom_density(alpha = 0.5) +
  theme_minimal() +
  scale_fill_brewer(palette = "Set1")
...
```{r}
chart_prev_quin <- ggplot(df_quin_prop, aes(x = adi_tertile, y = prop)) +
  geom_boxplot() +
  geom_jitter(width = 0.2, alpha = 0.6) +
  scale_y_continuous(limits = c(0, .6, .2)) +
  labs(
...
calculate_rr <- function(dataset_list) {
  ...
  for (dataset_name in dataset_list) {
    ...
    model <- glm(resistant_to_class ~ adi_tertile, data = data, family = poisson(link = "log"))
    # Compute robust standard errors
    model_results <- coeftest(model, vcov = vcovHC(model, type = "HC0"))
    # Extract the second coefficient (adi_tertileHigh vs adi_tertileLow)
    term_name <- rownames(model_results)[3]
    if (!is.na(term_name) && grepl("adi_tertile", term_name)) {
...
calculate_rd <- function(dataset_list) {
  ...
  for (dataset_name in dataset_list) {
    data <- get(dataset_name)
    model <- glm(resistant_to_class ~ adi_tertile, data = data, family = gaussian(link = "identity"))
    # Compute robust standard errors
...
```{r}
chart_prev <- ggplot(data = prev_by_adi, aes(x = grp, y = mean_prop, ymin = lower_ci, ymax = upper_ci, colour = adi_tertile)) + 
...
```{r}
prev_by_adi |>
  select(grp, adi_tertile, mean_prop, lower_ci, upper_ci) |>
  filter(adi_tertile != "Middle") |>
