...
```{r}
df1 <- df_staph |>
  select(isolate_id, pat_deid, culture_year, organism_name2, zip_code) |>
...
df_staph_county <- df_staph |>
  ...
  select(organism_name2, county_prop) |>
  add_count(organism_name2, county_prop) |>
  add_count(county_prop) |>
  distinct() |>
  mutate(pct_mrsa = round(n / nn * 100)) |>
  filter(organism_name2 == "mrsa",
         nn >= 10,
         !is.na(county_prop))

df_staph_zip <- df_staph |>
  left_join(zip_county) |>
  select(organism_name2, county_prop, zip_code) |>
  add_count(organism_name2, zip_code) |>
  add_count(zip_code) |>
  distinct() |>
  mutate(pct_mrsa = round(n / nn * 100)) |>
  filter(organism_name2 == "mrsa",
  ...
  select(-c(county_cap, organism_name2)) |>
...
roll_mean_num <- df1 |>
  group_by(organism_name2, culture_year, ses_quar) |>
  summarise(n = n()) |>
  filter(organism_name2 == "mrsa") |>
  group_by(ses_quar) |>
