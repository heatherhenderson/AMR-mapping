...
counties <- zip_county |> select(county_prop) |> distinct()

df <- entero_isolates_2014_2023 |>
  select(pat_deid, isolate_id, zip_code, mdr) |>
  distinct() |>
  left_join(zip_county) |>
  filter(!is.na(county_prop))

df1 <- df |>
  add_count(county_prop, mdr) |>
  add_count(county_prop) |>
  select(mdr, county_prop, n, nn) |>
  distinct()
...
fwrite(df2, "enterobacterales_mdr.csv")
```
