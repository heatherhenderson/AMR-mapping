...
```{r}
df_staph <- df3 |>
  filter(str_detect(organism_name, "staph"),
...
```{r}
df4 <- df3 |>
  select(pat_deid, isolate_id, culture_year, organism_name, antibiotic_class, nonsusceptible) |>
  distinct() |>
...
df5 <- df4 |>
  left_join(demo) |>
  select(original_file_mrn, pat_deid, isolate_id, birth_date, race_recode, ethnicity_recode, death_date, zip_code, parent_hospital, department_name, order_date, result_date, culture_year, isolate_source, organism_name, antibiotic, nonsusceptible, antibiotic_class, resistant_to_class, mdr) |>
  arrange(result_date, original_file_mrn) |>
  filter(!str_detect(organism_name, "species|enterobacteriaceae"))

names(df5)

n_distinct(df5$isolate_id)
n_distinct(df5$pat_deid)
# 105,164 isolates
# 79,396 patients

# write_rds(df5, "//ad.unc.edu/med/tracs/Groups/Research/CDWH/Van Duin_David_IRB23-2326//new_species_isolates_2014_2023.rds")
```
